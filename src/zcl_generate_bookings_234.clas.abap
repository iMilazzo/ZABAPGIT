CLASS zcl_generate_bookings_234 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_generate_bookings_234 IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF zim_booking234.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM zim_booking234.
*   insert the new table entries
    INSERT zim_booking234 FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM zim_booking234 INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
